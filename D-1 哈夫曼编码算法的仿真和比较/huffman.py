...
class HuffTree(object):
    ...
    def traverse_huffman_tree(self, root, code, char_freq):
        ...
        if root.isleaf():
            ...
            print(("data = %d : freq = %d ,code = %s")%(root.get_value(),root.get_wieght(), code))
...
def compress(data):
    ...
    compress_data.append(six.int2byte(a1))
    compress_data.append(six.int2byte(a2))
    compress_data.append(six.int2byte(a3))
    compress_data.append(six.int2byte(a4))

    # 4.2  每个值 及其出现的频率的信息
    # 遍历字典 char_freq
    for x in char_freq.keys():
        # output.write(six.int2byte(x))
        compress_data.append(six.int2byte(x))
        # 
        temp = char_freq[x]
    ...
    for key in char_freq.keys():
        freq[key] = char_freq[key]/filesize
    # print("freq:",char_freq)


    # 5. 构造huffman编码树，并且获取到每个字符对应的 编码,之后char_freq.values() 保存的是编码
    ...
    for i in range(filesize):
        ...
        while len(code)>8:
            ...
            compress_data.append(six.int2byte(out))
            out = 0

    # 处理剩下来的不满8位的code
    compress_data.append(six.int2byte(len(code)))
    out = 0
    ...
    ###
    # 计算平均码长  sum_(freq*code_length)
    average_code_length = 0
    for key in char_freq.keys():
        # print(key,char_freq[key],len(char_freq[key]))
        temp = freq[key]*len(char_freq[key])
        average_code_length += temp
    # print("平均码长:",average_code_length)
    return compress_data,average_code_length



def decompress(compress_data):

    filedata = compress_data
    # 获取文件的字节总数
    ...
    for x in range(leaf_node_size*5+4,filesize):
        #python3
        # c = filedata[x]
        c = six.byte2int(filedata[x]) # 转int
        for i in range(8):
            if c&128:
                code = code +'1'
            else:
        ...
        while len(code) > 24:
            if currnode.isleaf():
                ...
                decompress_data.append(six.byte2int(tem_byte))
                currnode = tem.get_root()
    ...
    if currnode.isleaf():
        tem_byte = six.int2byte(currnode.get_value())
        # output.write(tem_byte)
        # decompress_data.append(tem_byte) #byte2int
        decompress_data.append(six.byte2int(tem_byte)) 
        currnode = tem.get_root()
